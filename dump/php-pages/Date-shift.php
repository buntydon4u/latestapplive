...
if (isset($_GET['parentid'])) {
    ...
    $hisabview = '?login=' . $_GET["login"] . '&user_type=ledger';
}
?>
<!DOCTYPE html>
<html lang="en">
...
-------------------------- */
        ...
        /* Keep spacing good in desktop mode on mobile */
...
-------------------------- */
        ...
        /* Make sure buttons don't touch even in desktop-mode-on-mobile */
            ...
            <div class="container mt-5">
                ...
                <h4 class="text-white">Tanu SD</h4>
                            ...
                            <div class="card-header card-header-auth-1">
                                ...
                                <h4 class="text-white"><a href="hisablist.php<?= $hisabview ?>" style="color:white"><i class="fas fa-book" style="color:white !important" title="" name="ShowkaharIcon" id="showbtn"></i> Hisab</a></h4>
...
  <label for="javascript">Old Session</label>-->
				...
				WHERE `tbl_master_transaction`.`t_date` >= NOW() - INTERVAL 30 DAY AND `tbl_master_transaction`.`party_id` = '" . $userid . "' ORDER BY `tbl_master_transaction`.`id` DESC";
                                                            //echo $sql; die;	
                                                            $rs = $mysqli->query($sql);
                                                            ...
                                                            while ($row = mysqli_fetch_array($rs)) {
                                                                ...
                                                                //echo $ttime.' '.$time; die;
                                                                if ($ttime < $time && (date('Y-m-d', $ttime) == $t_date) && ($row['shiftid'] != 11)) {
                                                                    //if($ttime<$time){ 
                                                                    $count++;
                                                            ?><tr id="aall">
														...
														<td data-lable="Date"> -->
                                                                ...
                                                                <?php } elseif ($row['shiftid'] == 11 && $now >= $startDate && $now <= $endDate) {
                                                                ?>
                                                                    <tr id="aall" data-startdate="<?= $startDate ?>" data-enddate="<?= $endDate ?>">
                                                                        <td data-label="Date"><?= date('d M, Y', strtotime($row['t_date'])) ?></td>
                                                                        <td data-label="Shift Name"><?= $row['shift_name'] ?></td>
                                                                        <td data-label="Amt"><?= array_sum(explode(',', $row['trn_amt'])) ?></td>
                                                                        <td data-label="Action">
