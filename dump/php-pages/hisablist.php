...
<body class="background-image-body">
  ...
  <?php //echo 'ttp'; 
   ...
   if(isset($_GET['parentid'])){
	$home = 'https://appdemo.555xch.pro/Parent.php?parentid='.$_GET["parentid"].'&name='.$_GET["name"];   
   }
   else{
   $home = '#';
   }   
                        ...
                        <div class="form-group col-6">
                            ...
                            <select name="shift" id="shift"  class="form-control"
								...
								$ttime = time()+43200;
									...
									// $sql = "SELECT * FROM `tbl_final_hisab` WHERE `ledger_id` =".$_GET['login'];
...
//echo '<pre>'; print_r($date); die;
				...
				$sql = "SELECT * FROM `tbl_final_hisab` WHERE `ledger_id` =".$_GET['login']." ORDER BY STR_TO_DATE(date, '%d-%m-%Y') desc";				
                //echo $sql; die;	
									$rs = $mysqli -> query($sql);
									...
									while ($row = mysqli_fetch_array($rs)) { $count = 0;
										...
										<tr id="restt">
                                         ...
                                         <td style="padding-left:180px !important;padding-top: 10px !important;" data-label="<?=date('d M, Y', strtotime($row['date']))?>">
                                            <a href="hisab.php?ledger_id=<?=$_GET['login']?>&date=<?=$row['date']?>&updated_by=<?=$_SESSION['updated_by']?> "  class="badge badge-primary view-btn">View Hisab</a></td>
